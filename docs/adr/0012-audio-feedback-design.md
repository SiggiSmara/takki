...
- *Character drills (Layer 1):* Each character is announced individually by TTS, then the child types it. The next character follows after a correct keypress or a configurable timeout.
  ...
  **What the timeout does — resolved 2026-09-10 (alpha session 11; closes the "D auto-advance" carry-forward).** It **re-prompts**; it does not advance. Advancing would abandon the open prompt and put a different letter in front of a child who was still thinking about this one, with no signal that it happened — against the auto-reject model below, whose whole point is that the child is always at a known position with a clean sequence behind them. Re-prompting is bounded: after `PROMPT_MAX_REPROMPTS` (default 3, at `PROMPT_TIMEOUT_SECONDS` = 10 s each) Takki goes quiet and **the prompt stays open**, so a child who walked away is not read the same letter forever, and their first keystroke on returning is still a first attempt. The counting half was already settled and is compatible either way (alpha session 7): a timed-out prompt writes nothing at all, because `AttemptCounter` writes on keystrokes only. Critically, a re-prompt must **not** call `start_prompt()` again — re-latching would let the next keystroke count as a second first attempt (ADR-027 § Timeouts). The deadline is disarmed on entering PAUSED and re-armed on resume: monotonic time runs through a pause, so a deadline left armed is already in the past on the way back and would spend a re-prompt the child never sat through.
...
**There was a third case, and it fell between the two rows** *(fixed 2026-09-20, alpha session 12a-2)*: an utterance already **handed to the worker but not yet popped**. The core had let go of it, so clearing the pending sequence did not reach it, and it was not audible yet, so `stop()` did not either — it was spoken in full, after the interrupt, demonstrated as `spoken == ['LINE ONE', 'LINE TWO']` following a single `interrupt()`. The window is microseconds wide in Alpha, because the core enqueues one utterance at a time, and it widens the moment anything queues further ahead. `TTSWorker.stop()` now also raises a monotonic id threshold, `_cancel_through`; the worker reports any command minted at or below it as `cancelled` **without speaking it at all**. A threshold rather than a flag because a flag has to be cleared before each utterance, and every point at which it could be cleared reopened the same window. Nothing clears the threshold.

**A non-interruptible utterance is now genuinely non-interruptible** *(fixed the same session)*. `Speaker.interrupt()` stopped the outstanding *letter* before it consulted the interruptible flag, and for Alpha's `SyntheticLetterAudioSource` that stop **is** `TTSWorker.stop()` — so a milestone celebration was cut whenever a letter happened to be outstanding. The guard now comes first and nothing is stopped when the utterance in flight is non-interruptible, the letter included.
...
**Resolved 2026-09-10, alpha session 11: a prompt that becomes due while any utterance is still speaking is _held_.** Not dropped (the child would be left with no prompt and no way to know one was owed) and not spoken over (a letter landing on top of an introduction script teaches neither). The loop keeps "a prompt is due" as a flag and issues it on the first tick where nothing is audible, which makes the rule the same for the non-interruptible case this paragraph asked about and for the ordinary interruptible one — an introduction script, an auto-rejected re-prompt, a resume announcement. The residual cost is stated rather than hidden: while narration is speaking there is no open prompt, so a child typing ahead of it writes nothing (see roadmap § D, "keystrokes typed between prompts are dropped"). Inside a block that window is closed — the next prompt is issued in the same dispatch as the press that resolved the previous one.
...
*(Alpha session 11, 2026-09-10.)* **In Layer 1 there is no word**, so restart re-presents the current **unit** — ADR-024's bigram or short sequence, which is the nearest thing and which a drill block always ends on. The re-typed prompts **are** counted again: each is a fresh prompt and ADR-027 counts prompts, and "not counted toward session totals" has no Layer-1 mechanism behind it and no schema column to hold one. Filed in roadmap § D; if the pilot shows a child using restart to farm attempts, the fix is a decision about what a Layer-1 restart means, not a patch in the loop.
...
**Resuming from PAUSED re-issues the prompt — resolved 2026-09-10 (alpha session 11; closes the "D resume re-read" carry-forward).** Raised 2026-08-22 by alpha session 6b, which announces the resume and nothing else, leaving a child who task-switched away mid-drill to come back to silence and hunt for the re-read key — precisely the disoriented state this section exists to serve. The **loop**, not the focus model, now re-speaks the open prompt after the resume announcement: what a re-read says is engine state, so it belongs with the engine, exactly as this paragraph anticipated. Two properties make it safe. It goes through the same "re-speak the prompt that is still open" path as the re-read key, an auto-rejected keypress and the auto-advance timeout — one code path, so there is one place for the `start_prompt()` mistake to be made and it is not made there. And it is *queued*, not spoken: the loop holds it until the resume announcement finishes, so the letter never lands on top of "Back in Takki."

The child's re-prompt budget is also reset on resume — a pause is not evidence that they were ignoring the prompt.

**An introduction script cut by a focus loss is held and re-spoken whole — resolved 2026-09-20 (closes the "Interrupted introduction script" carry-forward).** The resume rule above covers a focus loss during a *prompt*. It said nothing about a focus loss while an **introduction script** is still speaking, and that case had two opposite failures, selected by thread timing: a line still in `Speaker._pending` was **dropped and unrecoverable**, while a line already handed to the TTS worker was **spoken after** the pause announcement. Both are reachable in one run — observed together, the first line of Stage 0's opening script spoke, the pause announcement followed, and the second key of the pair was never introduced at all. The likeliest trigger is the worst one: a window that comes up unfocused, whose seed `FocusLost` interrupts the session's very first script.

**On resume, the loop re-speaks the whole script**, queued behind the resume announcement, through the same `_speak_introduction` path the re-read key uses. Three decisions inside that:

- **Whole, not the remainder.** The child task-switched away and lost the context; a script resuming mid-sentence teaches less than one heard again. It is also what `KeyIntroducer.last_step` already makes available, so no remainder has to be tracked.
- **Only when a script was actually cut**, tracked by a flag set when the script starts speaking and cleared when the prompt it precedes opens. With no prompt open, "a script was cut" and "a celebration was playing" are otherwise indistinguishable, and re-speaking a stale introduction after a celebration would spend the child's only channel on something they did not ask for. The alternative considered and rejected was to make resume simply call the re-read path, which is a smaller diff and matches session 11's one-code-path principle, but cannot tell those two cases apart.
- **Not by making the script non-interruptible.** That would stop the focus loss cutting it, at the price of speaking the rest of the script *while Takki is in the background* and the child is somewhere else.

The prompt needs no special handling: `_advance` already holds it while anything is speaking, so the re-spoken script delays it naturally. Pinned by `tests/test_session.py::TestInterruptedIntroductionScript`. **The second half of the original defect is not fixed here** — a line already handed to the worker still speaks after the pause announcement, which is a cancellation defect in `TTSWorker.run_one()` and belongs to alpha session 12a-2. The rule above makes its consequence harmless (the script is re-spoken correctly either way), not absent.
