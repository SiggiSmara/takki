...
### Stage 0 — Anchor establishment

*(Added 2026-08-23. Precedes both phases below and is common to every introduction strategy.)*

Before any curriculum runs, the child learns to **find home by touch**. Stage 0 introduces exactly six keys — the two index home columns, positions (2,4) (3,4) (4,4) and (2,7) (3,7) (4,7), which are `r f v` / `u j m` on every QWERTY-derived layout in the v1 target set (German differs only at column 6, which Stage 0 does not use).

**Why these six.** F and J carry the tactile bumps moulded onto essentially every physical keyboard — the one orientation landmark a blind child can find without help, and the reference every later key position is described against (§ The key introduction script). Confining the drill to a single finger's home column means the finger must leave home and come back on every second stroke, so the stage *trains* reach-and-return rather than merely asserting a resting position. The stretch columns (`t g b` / `y h n`) are deliberately excluded: lateral displacement is a different skill and blurs the one thing this stage establishes.

**Why it precedes the curriculum rather than replacing Phase 1.** Anchor security is the base every ordering depends on, and it is the one part of the introduction order that no candidate ordering disputes — the home-row-fill order below starts at F+J, and so does the F/J-seeded alternative in [anchor-and-introduction-order.md](../research/anchor-and-introduction-order.md). Making Stage 0 a separate stage keeps it common to all of them, so choosing an ordering later does not disturb it.

**How the stage is emitted** *(added 2026-08-23, alpha session 8b.)* Stage 0 is three introduction steps, not one step of six keys and not six steps of one: each step is a symmetric pair by physical position — home `f`+`j`, then the reach up `r`+`u`, then the reach down `v`+`m`. Home is first because it is the position the other two are described against, and the pair shape is Phase 1's own, so nothing downstream needs a second shape for the stage: the left-hand member is first and [ADR-028](0028-composite-input-and-keyboard-ownership.md) § Pair ramp-up reads the pair straight off the step. What marks the stage is `stage = 0`, which the drill generator needs anyway to select Stage 0's content. *(Field renamed from `phase` 2026-08-23 by [ADR-032](0032-grapheme-led-introduction-and-selectable-ordering.md) § What this changes item 3: Phase 1 and Phase 2 belong to one candidate ordering, so a field every ordering shares cannot be named after them. The field now carries `0` for this stage and `1` for the whole curriculum below it.)*

**The six keys stay ordinary curriculum keys.** `f` and `j` open the default strategy's Phase 1 and `r v u m` are Phase 2 keys, so the sequencer hands them to the strategy as already-introduced rather than deleting them from it — a strategy stays a complete ordering in its own right and needs no knowledge that Stage 0 exists. The fully-consumed F+J slot emits no step at all rather than a degenerate empty one.

**Stage 0 gates the ladder's first rung.** Completion is measured by first-press accuracy on the six keys at the anchor bar; the criterion, and why plain accuracy is a valid anchor measure here when it was not for the old Bronze, are in [ADR-027 § The Anchor Gate](0027-key-and-accuracy-state-model.md#the-anchor-gate).

### The introduction order is a swappable strategy

*(Added 2026-08-23.)* Phases 1 and 2 below are **one strategy**, and the default. They are not the only one this ADR admits.

[anchor-and-introduction-order.md](../research/anchor-and-introduction-order.md) (2026-06-07, with a confirming order-comparison spike on 2026-06-14) measured five candidate orderings across seven languages and found the home-row-first Phase 1 costs 9–10 keys before the child's first off-home reach, against 2 for an F/J-seeded order — and that the real-word onset penalty is modest in English but large in vowel-poor layouts (French: 11 keys versus ~5). Its recommended ordering, O5, is not adopted here: the note is explicitly *pre-decision*, and it assigns the choice to the Beta pilot, because the outcome that would settle it is a child-specific motor result nobody has measured yet.

What follows from that is a structural requirement on the implementation, not a change of default: **the introduction order after Stage 0 must be selectable, so the pilot can A/B home-row-fill against an F/J-seeded order and against a per-child calibrated order without an engine rewrite.** Hard-coding one ordering would make Alpha an arm of the experiment it is supposed to be the instrument for. The default stays the two-phase order below until the pilot says otherwise; see open question 6.

**The seam that shipped** *(added 2026-08-23, alpha session 8b.)* A strategy is a callable `(layout, source, had) → list[IntroductionSlot]`: the order below Stage 0 as key *names*, before finger and location are worked out, which keeps the introduction script, the modifier rules and the location metric in one place across every ordering. The selection is a **constructor argument** on `KeyIntroducer`, defaulting to the two-phase order below — not a config key and not a `profiles` column. The pilot A/Bs orderings per child, which is [ADR-025](0025-configuration-system.md)'s per-profile tier; that tier does not exist yet, a global config key would be the wrong tier and would have to move, and a schema column has nothing to store in it until the pilot picks. The session loop already holds one `KeyIntroducer` per session (§ What an introduction step is, and what the introducer remembers) and is where the choice is made. One rule binds every strategy: it filters `had` itself, because pairing happens after filtering — a strategy that ignored `had` would mispair the survivors, not merely repeat itself.

### Phase 1 — Home row, symmetric pairs by physical position

> **Superseded by [ADR-032](0032-grapheme-led-introduction-and-selectable-ordering.md) § Ordering A (2026-08-23).** This is not *the* order; it is the first phase of one candidate ordering, kept as Alpha's default because it is the pilot's control arm. Its location-anchoring justification below has been taken over by § Stage 0, and the modifier rules in this section are void — a modifier no longer appears in any ordering, so the `dead-acute`-at-step-18 placement and its 8b renumbering are both moot. The mechanics — symmetric pairs by column, solo partners, the outward tail — are unchanged and still describe Ordering A.
...
> **Superseded by [ADR-032](0032-grapheme-led-introduction-and-selectable-ordering.md) § Ordering A (2026-08-23)** on two points: the pools rank **graphemes**, not physical keys, and a modifier is never among them. The rest — the positional phase boundary, the leader-per-hand rule, solo steps draining both pools, and the argument against pure single-key frequency — is unchanged and still describes Ordering A.
...
**Interaction with the milestone ladder.** *(Revised 2026-08-23.)* No milestone gates this boundary. Bronze's "home row known" was retired in [ADR-027 § Milestone Ladder](0027-key-and-accuracy-state-model.md#milestone-ladder), and the ladder's first rung is now the anchor gate on Stage 0 (below), which sits *before* the curriculum rather than inside it. Progression past the home row is governed by ADR-010's accuracy gate alone. [ADR-028](0028-composite-input-and-keyboard-ownership.md) § Pair ramp-up already assumes this: its Layer-2 unlock arithmetic reaches 9 Active English home-row keys and unlocks immediately, with no Known requirement anywhere in the count.
...
**What a step carries is a grapheme, and what the record holds is grapheme names** *(added 2026-08-23, alpha session 8c, implementing [ADR-032](0032-grapheme-led-introduction-and-selectable-ordering.md) § Decision 1).* A member of a step is a letter the child produces together with the keys it takes to produce it, so `á` is one member carrying two keystrokes. The record below is therefore keyed the same way `key_stats` is — by grapheme — and the two stay comparable, which is what lets a letter leave the record for good the moment it becomes Active.

**Whether a modifier has already been announced is derived, not recorded** *(added 2026-08-23, alpha session 8c).* A modifier rides in on the first composite of its class ([ADR-032](0032-grapheme-led-introduction-and-selectable-ordering.md) § Decision 1), so something has to know whether that has happened. It is **not** a second session-local set: the introducer already runs a set of the physical keys behind every grapheme the child has, because the location metric needs a pool of keys to point at, and the modifier enters that set exactly when the first composite carrying it is introduced. "Has this modifier been announced" is then the same question as "is this key already behind something the child has", and there is nothing to keep in sync.

The two designs differ in exactly one case — a composite introduced and never answered — and the derived answer is the wanted one. Such a composite has no `key_stats` row, so a fresh introducer re-introduces it next session, and the derivation re-announces the modifier along with it. That is correct rather than merely convenient: the introduction script is that letter's only teaching moment, and the modifier clause is a clause *of* that script. A record that survived the letter would drop the child into a composite they were never told how to type. A *persistent* record would behave differently again, and would need a schema change ADR-011 does not have.

The mechanism explanation is a separate question from the key, and is answered separately: the words *"it changes the next letter you press"* are spoken once per mechanism class, while the name and location are spoken once per modifier key. The two coincide in every layout in the v1 target set and come apart in a layout with two dead keys — two new keys, one idea.
...
~~**The consequence for modifiers, and why it is not fixed here.** A letter leaves this set and stays gone, because answering its first prompt makes it Active. A modifier never can — that is precisely B8 — so a session-local record suppresses it for one session and nothing suppresses it after that. The Icelandic sequence therefore re-announces the accent key at the head of the right-hand pool in every session, displacing a real right-hand key each time, and repeats it forever once the rest of the sequence is exhausted.~~ **Closed 2026-08-23 by [ADR-032](0032-grapheme-led-introduction-and-selectable-ordering.md), implemented in alpha session 8c.** A modifier is no longer in the set because it is no longer in the sequence: nothing introduces it in its own right, so nothing has to retire it. The pin test this paragraph named (`test_a_modifier_repeats_every_session_until_roadmap_b8_is_resolved`) is deleted, and the test standing in its place asserts the opposite — that the sequence terminates and `dead-acute` never appears as an ordered item.
...
A grapheme is **typeable** once all its prerequisite physical keys are known. ~~The Phase 2 sequence orders *physical keys*; composite graphemes become available implicitly when their last prerequisite is introduced.~~ *(Second sentence superseded by [ADR-032](0032-grapheme-led-introduction-and-selectable-ordering.md) § Decision 1, 2026-08-23: the sequence orders **graphemes**, and a composite is introduced at its own frequency rank rather than appearing implicitly. The data model above is unchanged and is what makes that possible — it is the layer that knows which keys a letter needs.)*

> **Superseded by [ADR-032](0032-grapheme-led-introduction-and-selectable-ordering.md) § Decision 1 (2026-08-23).** A modifier has no frequency, no rank and no step. It is announced inside the step that introduces the first composite needing it. The aggregate-composite-frequency score below existed only to give a modifier a place in a key-ordered sequence, and the sequence is now grapheme-ordered. The paragraph is kept as the record of what was tried; the spike's step-12/step-18 placements remain accurate as statements about the spike.
...
**The introduction script for composites.** *(Trigger superseded by [ADR-032](0032-grapheme-led-introduction-and-selectable-ordering.md) § Decision 1, 2026-08-23: a composite **is** introduced at its own step, at its own frequency rank, and the modifier rides in on the first composite that needs it. The script below is unchanged and is what that step plays — only the moment it fires has moved, from "the first time drill content happens to surface the letter" to "the step that introduces the letter".)* The script extends the per-keypress prompt from [ADR-012](0012-audio-feedback-design.md):
...
**Milestone denominators.** The milestone ladder's rungs count distinct typeable **graphemes** (output characters) known — not physical key actuations. Modifier keys (AltGr, dead keys) produce no character on their own and are excluded from the denominator; the characters they help produce (`á`, `ð`, `ž`, etc.) are included on equal footing with direct-strike characters. This is decided in [ADR-027](0027-key-and-accuracy-state-model.md), which supersedes the earlier "physical keys" wording in this ADR and, from 2026-08-23, defines the six-rung ladder those thresholds belong to. For Icelandic: the denominator is the 32 output graphemes from `get_layout_positions()`, not the 24 physical keys; a child who has mastered 14 graphemes (= 14/32 = 44%) is measured against the full grapheme set.
...
6. **Which introduction order the curriculum uses after Stage 0.** *(Raised 2026-08-23; moved to [ADR-032](0032-grapheme-led-introduction-and-selectable-ordering.md) open question 1 the same day, where the candidates are set out as siblings. Kept here as a pointer.)* The two-phase order in this ADR is the default and is implemented as one selectable strategy (§ The introduction order is a swappable strategy). [anchor-and-introduction-order.md](../research/anchor-and-introduction-order.md) recommends an F/J-seeded order (O5) on measured anchor cost and real-word onset, and proposes a per-child middle-finger calibration probe — the observation that a child's middle finger may rest more naturally on E than on D, which columnar-stagger ergonomics and a finger-length/typing-speed correlation both support. That note is pre-decision by its own status line and assigns the choice to the Beta pilot; its own next steps put the ADR amendment *after* the pilot, not before. Resolve it there, then amend this ADR. Note the ripple the note flags: any milestone or drill rule phrased in terms of "the home row" has to be re-read under an F/J-seeded order — [ADR-027](0027-key-and-accuracy-state-model.md)'s ladder was already de-positionalised for this reason.
...
- **2026-08-23 (alpha session 8b follow-up).** Ordering and modifier placement lifted out of this ADR into [ADR-032](0032-grapheme-led-introduction-and-selectable-ordering.md): §§ Phase 1 and Phase 2 are one candidate ordering of three, introduction is grapheme-led, and a modifier no longer has a step. Superseded passages are marked in place rather than rewritten — the framing that presented one ordering as the protocol is what roadmap B9 diagnosed, so the record of it is kept deliberately. Everything not marked stands.
...
- **2026-08-23 (alpha session 8 follow-up).** Stage 0 added as a pre-curriculum anchor-establishment stage on the six index home-column keys, gating the milestone ladder's first rung; the two-phase order below it demoted from *the* order to the default *strategy*, with selectability required so the Beta pilot can A/B it; the orphaned anchor research note wired into the open questions as item 6; the Bronze interaction paragraph replaced, Bronze having been retired from ADR-027's ladder.
