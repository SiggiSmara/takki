...
**The comparison is also blind to graphemes, and that is an omission rather than a decision.** *(Amended 2026-09-20, alpha session 12a-1, which built the reader this section was written in anticipation of.)* `describe_mismatch` compares `key_positions()` — `keys` only. But `Layout` carries two dictionaries, and `graphemes` is the one the engine teaches from: it filters the word pool (`language/__init__.py` `_eligible_words`) and it is the milestone denominator ([ADR-027](0027-key-and-accuracy-state-model.md) § Milestone Denominator). So a layout can pass this check and still hand the engine the wrong curriculum. The two sets diverge exactly where a language uses dead keys: a real Icelandic keyboard reads 31 keys against 36 graphemes, because `dead-acute` is a key and not a grapheme while `á é í ó ú ý` are graphemes and not keys — six characters the check never looks at.

The empirical question that was deferred until a reader existed is **answered: yes, a scan-code sweep produces trustworthy composite data.** Measured 2026-09-20 against a real Icelandic layout, the six acutes come back matching `build_is()` field for field — `mechanism`, `prereq_keys`, `keystrokes`, `base`, `dead_key`. So extending the comparison is now a small change rather than a research problem.

**Extend it alongside the position check, never instead of it.** Graphemes carry no position, and `en` and `de` both contain `y` and `z`, so a grapheme-set comparison cannot see QWERTY versus QWERTZ at all — the one case this section exists for. Positions catch a moved letter; graphemes catch a missing or surplus one; neither subsumes the other.

**Blocked on [roadmap § C16](../roadmap.md#c-genuinely-unhandled-corner-cases-mostly-voicebeta-but-cheap-to-decide-now), and this is the reason it is not simply fixed.** If the alphabet becomes a language-layer input distinct from what the keyboard can produce, then "compare the graphemes" has two possible meanings and they are not the same check: *can this keyboard produce everything the curriculum teaches* (the one that matters, and a question this section does not currently ask at all), versus *does this keyboard produce exactly what the table predicted* (what a naive extension would assert, and what would reject a perfectly usable keyboard for carrying an extra accent). Decide C16 first; the shape of this check follows from it.

**Empty for Alpha either way.** `_direct_layout()` writes `keys` and `graphemes` from one loop, so `build_en()`'s two dictionaries are the same 26 names and `build_de()`'s the same 30 — for English the position check already implies the grapheme check. Nothing ships wrong today; the gap opens with the first dead-key language.
