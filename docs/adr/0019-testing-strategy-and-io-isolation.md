...
- **Headless audio/video.** `SDL_AUDIODRIVER=dummy` and `SDL_VIDEODRIVER=dummy` let `pygame` initialise without a sound card or display. Catches code-path regressions; humans verify quality.
  ...
  **Closed (2026-09-24, alpha session 12a-2).** What was left open above is now decided and built, and the probe job is gone — a question answered is not a reason to keep asking it.
  ...
  - **The refusal is proven for the voice, not for `main()` (2026-09-24, alpha session 12b-1, found by reading the code, not by running it).** `tests/test_no_audio_output.py` drives `get_fallback_tts` → `TTSWorker.start()`. `main()` constructs `PygameMixerCues()` *before* that, and `pygame.mixer.init()` is not guarded. On the runner the mixer opens because the job sets `SDL_AUDIODRIVER=dummy`. On a real machine with no output device, SDL has no device to open and probably raises `pygame.error` first, which would mean a traceback and exit 1, never the exit-4 message. The same blind spot as this section's other instances: the dummy driver supplies the device the check is about. Settled by hand in windows-validation.md A4c (its *Watch for*); if it reproduces, the fix is ordering or catching in `main()`, not in the test.
