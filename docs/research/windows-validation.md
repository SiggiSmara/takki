...
**At the laptop, use a run sheet.** A run sheet is *Running order* rendered as test cases: each step with its exact command, its pass condition copied from the tier row, and a place to write the result. **Generate a new one for every run**, in a folder of its own named after the date it was generated: `windows-validation-runs/YYYY-MM-DD/runsheet.md`. Add the folder to § Runs. Every raw file the run writes goes into that folder too, **named after the run-sheet test case that records it** (for example `RS-06.log`), one file per trace run. `.gitignore` lets `*.log` through there, and only there, so the evidence is committed next to the sheet it belongs to. **Raw logs are linked from the sheet, never pasted into it.** Short outputs a step asks for (a result line, a dump, the C7 verdict) are still written into the sheet. In this note, `<run>` stands for that folder and `<tc>` for the test case's ID. **The run sheet is the single source of truth for its run.** Results are recorded there and stay there: nothing is copied back into this note. This note is the source of truth for the *protocol*, and a sheet adds no steps or pass conditions of its own. A sheet generated before this note last changed is stale; generate a new one rather than editing an old one.

**The instruction for an agent generating a run sheet:** render *Running order* as one test case per step or step group, in the same order and with the same step numbers (S1–S61). Repeat the full command in every test case, even when it is the launch line. Copy the pass condition from the check's tier row. Give each record field a line to fill in, and add a block to paste raw output into (trace sections, C7 output, dumps). Replace every `<run>` with the run folder's repo-relative path and every `<tc>` with the test case's ID. Carry the stop rules and restores inline. Record the generation date and the commit of this note in the sheet's header. End with a summary table of every check ID, hard no-go checks marked, and a line for the run's one-line finding. **Within a test case, list actions in the order they are done, starting with the one that starts the sequence** (the first RS-07 put "start the stopwatch" after the list it starts). **Record time only where a check uses it:** each sitting's date (D4 and D6 depend on calendar dates), and the timings a check measures (B2, A4c's time to exit, D4's progress readings, F1's start and end, E10's close). Leave out start and end times for sittings and for practice stretches such as Stage 0; no check reads them. Give each sitting a free-text line for interruptions instead. *(Added 2026-09-26, #12b-2: the first run sheet carried sitting start and end times, and a one-hour investigation of C6 mid-sitting made them meaningless as a guide to sitting length.)*
...
6. **The commands used below, in one place:**
   ...
   | Key trace (C) | `uv run python spikes/pynput_trace_spike.py <run>/<tc>.log`, then Ctrl+C in that console to stop it. Each run appends a new section. |
   | Trace vs dump (C7) | `uv run python spikes/c7_trace_vs_dump.py <run>/<tc>.log` |
...
12. **First, practise C2's order in Notepad a few times** (step 14): Shift down, `g` down, Shift up, `g` up. Then, in the second console, start the key trace. Click into Notepad.
13. **C1.** Hold `f` for about 2 seconds, then let go.
14. **C2.** Hold Shift. Press `g`. Let go of **Shift first**, then `g`. Letting go of Shift before the letter is not how anyone types, and it takes concentration to get the order right. That is why step 12 has you practise it first. Even then, `g` is often held past the ~500 ms auto-repeat delay while you get Shift up; that adds a repeat line and is fine (C2's row).
15. **C4.** Type `l`, release, `l`, release.
16. **C5.** Press `l` and hold it for about 1 second, then let go.
17. Press Ctrl+C in the trace console. Read the new section with `$t = Get-Content <run>/<tc>.log; $i = ($t | Select-String '=== trace started' | Select-Object -Last 1).LineNumber; $t[($i-1)..($t.Count-1)]`. The C1 hold alone can produce more than 60 repeat lines, so `-Tail 60` would cut it off. Record C1, C2, C4 and C5. The log stays in the run folder, linked from the sheet.
18. **C6, with its own trace started on Icelandic.** The trace translates keys with the layout it started under and never follows a switch (C6's row), so it must *start* on Icelandic. In the trace console, type `uv run python spikes/pynput_trace_spike.py <run>/<tc>.log` **on US** but do not press Enter: Icelandic moves `/`, `_` and `.`. Press Win+Space and pick **Icelandic**, then press Enter. Click into Notepad. Press the key right of `æ` (the dead acute, where US has the apostrophe), then `a`. Notepad should show `á`. Press Ctrl+C in the trace console. **Press Win+Space and pick English (US).** Read it with the command in step 17, on this step's log. Record C6 and what Notepad showed. The log stays in the run folder, linked from the sheet.
...
20. Have a stopwatch ready. Press Enter on the launch line and start the stopwatch at the same moment. **Close your eyes.** G6 runs from here until you hear the *second* introduction script.
    ...
    - **B11–B14:** Escape gestures. None of them count as attempts. **B12 and B13 cannot be judged by ear here:** In Stage 0's first step a unit is one letter, so a restart replays the letter just asked and sounds exactly like a re-read. Record what you heard; the pass is judged in step 25.
...
25. Practise until the end of Stage 0: all of `r f v u j m` have been introduced, and the next introduction names a letter outside those six. Answer correctly. That is D1's ~360 prompts. Mid-way, press one wrong key once (deliberate error 2 of 2) to re-check B6. **Once `r` and `u` have been introduced, re-test B12 and B13.** From Stage 0's second step each new letter is drilled right after its anchor, in two-letter units (`f r`, `j u`, then `f v`, `j m`), so a restart audibly goes back to the anchor. While being asked the second letter of a unit: hold Escape past 800 ms (B12: the anchor is asked, key still down); release at ~700 ms (B13: the same letter again); hold to ~900 ms (B13: the anchor). Record how many tries each side of 800 ms took.
...
37. **D4.** Launch and practise, answering correctly. Stage 0 is over, so the engine mixes the anchors in with newer letters. **Expect a new letter to be introduced in the first block:** a new session has no ramp-up in progress and the introduction gate is slack by now (alpha-plan #12d). That is a known defect, not a D4 failure; answer the new letters normally.
38. Every ~10 minutes, run the dump. You need each of `r f v u j m` to have a row dated **today**, and, **over each key's last 200 attempts across both days together**, at least 25 attempts and at least 95% accuracy (ADR-027; `window_stats` counts the key's rolling window, not one day). After a full Stage 0 on day 1 every anchor usually has its 25 already, so **one correct answer per anchor today is typically enough.** *The trap:* the window is the last 200 attempts per key, so an anchor answered more than 200 times today before the rung fires loses its day-1 attempts, has one date left, and cannot qualify until another day. Common anchors (`r`) get there in roughly half an hour of the mixed drill, so check the dump every ~10 minutes as this step says, and stop once `anchor` is confirmed. *(Clarified 2026-09-26, #12b-2: the old wording read as 25 attempts on day 2 alone.)*
...
| B12 + | Restart key (Escape held past 800 ms) | Fires **once**, at the threshold, with the key still down. The current unit is presented again. **Judged in step 25, not step 20.** A restart replays the unit from its start, and in Stage 0's first step a unit is one letter, so a restart sounds exactly like a re-read. From the second step units are anchor + new letter, and a restart on the new letter goes back to the anchor. *(Added 2026-09-26, #12b-2: the first run could not tell the two apart in step 1.)* | — |
| B13 − | Escape tap at ~700 ms and ~900 ms | 700 → re-read, 900 → restart. Record how hard the boundary is to hit. Carry-forward **D Escape** chose one key on purpose, and this is the check on that choice. **Judged in step 25**, for the same reason as B12 | — |
...
| C2 + | Hold Shift, press a letter, **let go of Shift first**, then the letter. Practise the order a few times before tracing (Sitting 1 step 14) | Expected: `PRESS` shift, `PRESS char='G'`, `RELEASE` shift, `RELEASE char='g'`. The release reports the **same** key in the other case, and that is a pass, because case is folded on both sides. **Also a pass:** one or more `PRESS char='g'` lines between the Shift release and the `g` release. Those are auto-repeats (the first one comes ~500 ms after the `G` press, the same delay as C1's). They are lower case because Shift is up by then. Folded, they are presses of a key already down, so they are repeats and not actuations ([focus_model.py `_on_key`](../../src/takki/focus_model.py)). Releasing Shift first is unusual and takes concentration, so a tester who has not practised it will often hold the letter past the repeat delay. **Fail:** `None` or an unrelated character on the release (it leaks a down-entry), or a `RELEASE` of the letter before its last `PRESS` (every repeat would then read as a fresh actuation). *(Corrected 2026-09-24: the step used to say "press Shift, release it, then press and release a letter", which never produces the mismatch the check is for. Injected input in #12b-1 shows `'F'` down / `'f'` up. Corrected again 2026-09-26, #12b-2: the hardware run showed `'G'` down, one `'g'` repeat 509 ms later, then `'g'` up, which the old wording did not allow for.)* | — |
...
| C6 − | What Takki's capture delivers for a dead-key letter (trace only, no lesson engine). **The Icelandic layout `0x40f` is already installed.** The dead acute is the key right of `æ` (position (3,11), US apostrophe). The trace must be **started with Icelandic already active** (Sitting 1 step 18) | **Record the lines.** Expected, on a trace started on Icelandic: `PRESS char='´'`, then `PRESS char='a'`, and Notepad shows `á`. **A composed `á` never arrives, and that is not a defect of the check.** Windows composes a dead key with the next letter inside the app receiving the keys, *after* the low-level hook pynput reads. pynput does flag the `´` as a dead key, but `translate()` in `pynput_stream.py` keeps only `char`, so Takki would read the dead key as an ordinary typed character. Both points are Beta findings for dead-key languages (roadmap § D), not Alpha defects. If the trace shows `"'"` instead, the trace did not pick up the layout it started under: record that, because Takki's listener gets its layout the same way. Capture only, by roadmap scope: does not gate Alpha's English run. *(Corrected 2026-09-26, #12b-2. The row used to expect one composed `á`, and the step switched to Icelandic with the trace already running. That run logged `"'"` then `a` while Notepad showed `á`. pynput's `KeyTranslator` reads the layout of its own listener thread and refreshes it only on `WM_INPUTLANGCHANGE`, which a hook thread with no window never receives, so a running trace stays on the layout it started with.)* | — |
...
| E12 − | Mid-lesson layout switch: Alt+Shift or Win+Space to German, then back (Sitting 1 step 28) | **Known gap, record what happens.** Takki verifies the layout at startup only ([ADR-025 § Language and layout must agree](../adr/0025-configuration-system.md), roadmap § D "A layout switched mid-session is not detected"), so the expected result is that nothing is said and the lesson carries on against the German keyboard. Anchor keys are unaffected; `y`/`z` and the umlaut keys would not be. Record which chord switched and whether anything was heard; if Takki *does* notice, that is news for the roadmap line. **Which mapping Takki reads keys with after the switch is a separate question, and the answer may be US.** Takki's pynput listener works like the C6 trace: it keeps the layout its thread started with and never hears a switch. If so, Takki goes on reading keys with the US mapping while every other app follows German. Observed on 2026-09-26 with the key trace, which uses the same `PynputKeyStream` as Takki: a trace started on US and left running across a switch to Icelandic went on logging US characters (C6's first run, [roadmap A5](../roadmap.md#a-bites-alpha-specifically-the-next-step)). Not yet observed in Takki itself. This step cannot observe it: a key that differs between the layouts (`y`/`z`) is never prompted in Stage 0, and pressing one is a wrong answer that counts against the anchor (Ground rule 3). *(Added 2026-09-24, #12b-1, after the developer pointed out that a layout can change during or before a run without anyone meaning to.)* | — |
...
|---|---|---|---|
| [windows-validation-runs/2026-09-26/](windows-validation-runs/2026-09-26/runsheet.md) | 2026-09-26 | #12b-2 | _not yet run_ |
