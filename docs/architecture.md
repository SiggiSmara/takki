...
| **The rest of the keyboard** — Shift and capitalisation, punctuation, the space bar, navigation and editing keys (arrows, Home/End, Tab, Delete) | Out of scope for v1, and the boundary is deliberate: Takki teaches *typing* — which key a letter lives on — not the keyboard as a device. The 2026-09-20 case-folding decision ([ADR-027](adr/0027-key-and-accuracy-state-model.md)) follows from it: an upper-case answer is the right letter, because the modifier that produced it is not something Takki taught or tested. **The open question this leaves is genuine and larger than a scope note.** Keyboard navigation is a real access skill for a VI child — arrowing through a document, Home/End, Tab between fields — and the frameworks that mandate touch typing ([CFVI Area 8](#educational-grounding), the ECC) name it in the same breath. Nothing in the architecture precludes it: it would be new curriculum layers above Layer 2, using the same drill engine, progression model and audio stack. **Deferred as a question, not a rejection, to be revisited once there are full releases behind us and the project has a user base and interest to justify the scope.** Asking it earlier would trade a finished typing tutor for an unfinished keyboard course. |
