...
1. **ADR-027 "Key & accuracy state model"** — the introduced→known→mastered lifecycle, first-attempt counting semantics, and reconciling Bronze with the Alpha schema (A2, A3, B6).
2. **ADR-028 "Composite input & keyboard ownership"** — dead-key/AltGr drilling, compose-state flushing, focus/suppression, the keypress taxonomy, and the pair-vs-single ramp-up integration (B5, B7, C8).
