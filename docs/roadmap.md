...
- Milestone ladder + Stage 0 anchor gate detection (see [ADR-027 § Milestone Ladder](adr/0027-key-and-accuracy-state-model.md#milestone-ladder))
...
- Milestone *names* and the spoken progression narrative (slugs only in Alpha; naming is tuned with real children in Beta)
...
6. Lesson engine: Layer 1 drill generator (per [ADR-024](adr/0024-drill-content-and-lesson-granularity.md) — four-phase ramp-up, freq-weighted bigrams, rare-key re-exposure, child-pace-adaptive drill blocks), Stage 0 anchor establishment then the adaptive key introducer (per [ADR-023](adr/0023-key-introduction-protocol.md) — the order below Stage 0 is a swappable strategy defaulting to home-row symmetric pairs then frequency-leader-per-hand, with spoken intro script), progression thresholds, milestone-ladder detection including the Stage 0 anchor gate
...
- **Alpha done:** the core-loop logic runs green on Linux against fakes/scripted I/O, and on Windows a dev can (a) complete **Stage 0** end-to-end in English — the anchor drill on `r f v` / `u j m`, the anchor milestone firing on completion, then close the app, reopen, and see progress restored — and (b) pass a hand-run input-assumption smoke check: the held-key/auto-repeat rule (`ll` typed with and without releasing between presses), a dead-key composition delivering a single composed character at the capture boundary, and the gesture keys that Alpha actually binds — re-read tap, restart hold, and pause/resume on focus loss. *(Revised 2026-08-23. The previous criterion was "a Bronze-level English drill session"; Bronze was retired from the ladder, and reaching it would have required roughly 810 hand-typed prompts across two days before Alpha could be declared done. Stage 0 tests the thing Alpha claims to prove — that the loop works with a real person — and is demonstrable in a sitting or two. Bronze's replacement, the six-rung ladder, is still implemented and unit-tested; nobody has to grind to it by hand.)*

  Two scope notes on (b). The dead-key check is **capture only** — that pynput delivers `KeyCode(char='á')` at the input boundary — run with no lesson engine involved. Alpha stays English-only and teaches no composite, so this needs neither multi-language support nor a resolution to **B8** below. And the **talk key has nothing behind it in Alpha**: push-to-talk and Whisper are Beta, so "the system keys" here means re-read, restart, and pause/resume only.
...
**Spiked (2026-06-28, Windows laptop) — see [research/tts-letter-pronunciation.md](research/tts-letter-pronunciation.md).** The assumption inverts: **SAPI** says letters *cleanly* (en/de) but only for installed Windows voices (no Icelandic); **Piper (neural)** is *structurally bad* at isolated letters at every tier — the phonemes espeak hands it are correct, but the VITS model distorts 1–3 phoneme utterances, and no spelling/SSML/tier trick fixes it. **espeak-ng** (rule-based, driven directly via its bundled DLL) says letters reliably across 100+ languages incl. Icelandic, at the cost of robotic timbre. Recommendation in the note: treat letters as a **closed curated asset set** behind a `LetterAudioSource` Protocol (recorded → bundled-per-language → runtime-TTS fallback), not runtime synthesis — which narrowly reopens [ADR-003](adr/0003-text-to-speech.md) (letter names) and largely moots the ADR-009 map. Minimum move for en+de Alpha: letters → SAPI, Piper for connected speech. Larger `LetterAudioSource` build is the open decision.

**A2. Bronze's "sustained across multiple sessions" is not computable from the Alpha schema, and the key lifecycle is undefined.** *(Historical — resolved by ADR-027 as recorded below; note that **Bronze itself was later retired** on 2026-08-23 in favour of the six-rung ladder, so the Bronze definition discussed here no longer exists. Retained for the key-lifecycle half, which stands.)*
...
**Spiked (2026-06-28) — see [research/tts-letter-pronunciation.md](research/tts-letter-pronunciation.md).** SAPI `stop()` from a second thread cut a ~12s utterance at ~2.2s on the test laptop. The worst case did not materialise; the relax-the-rule fallback is likely unnecessary. Confirm by ear on target hardware before relying on it.
...
**C15. Feedback density never adapts: the child hears the same chime on their 10th keypress and their 10,000th.** *(added 2026-08-22)*
*Phase: Beta or later (needs ADR-027 accuracy window + ADR-024 pace). ADRs: [012](adr/0012-audio-feedback-design.md), [027](adr/0027-key-and-accuracy-state-model.md), [025](adr/0025-configuration-system.md).*
Constant per-trial feedback helps acquisition and hurts retention (guidance hypothesis), and at high accuracy the `correct` chime carries almost no information while occupying the child's only channel. Three interlocking pieces are proposed in [ADR-012 § Open questions and future work](adr/0012-audio-feedback-design.md#open-questions-and-future-work): fade the `correct` cue as proficiency rises (per-profile, suggested at a natural endpoint, triggered on accuracy/pace rather than on cue overlap, keeping the cue for not-yet-Known keys); add a short acknowledgement click so "the keystroke reached Takki" stays audible when `correct` is suppressed; and cap configurable cue duration. Nothing here changes the `SoundCuePlayer` surface — suppression is the engine declining to call `play()` — so it does not gate Alpha. The supporting literature is not yet summarised in `docs/research/`; that write-up should precede any ADR amendment.

**B8. ~~A modifier key can never become Active, so no composite ever becomes typeable.~~ RESOLVED 2026-08-23 by [ADR-032](adr/0032-grapheme-led-introduction-and-selectable-ordering.md)** *(raised 2026-08-22, alpha session 7)* — introduction is now grapheme-led: a modifier has no step and no rank, it is announced inside the first composite that needs it, and the composite itself is the prompt target that acquires the `key_stats` row. The state the entry asked about is gone rather than redefined, which is what its own warning against "a second definition of Active" required. Both consequences below are closed with it, including session 8's forever-re-announced accent key; the pin test named at the end of this entry was deleted by alpha session 8c (2026-08-23), which implemented ADR-032; the test standing in its place asserts the opposite — that the sequence terminates and `dead-acute` never appears as an ordered item. The history is kept because the *shape* of the failure — three individually-right rules that are jointly unsatisfiable — is the thing worth recognising again.
*Phase: bites the first dead-key language (Beta/V1); does not block Alpha or German. ADRs: [027](adr/0027-key-and-accuracy-state-model.md), [028](adr/0028-composite-input-and-keyboard-ownership.md), [023](adr/0023-key-introduction-protocol.md).*
[ADR-028](adr/0028-composite-input-and-keyboard-ownership.md) § Modifier introduction gates composite availability on "both modifier and base letter have `key_stats` rows". [ADR-027](adr/0027-key-and-accuracy-state-model.md) creates that row on the first counted keystroke against the character as a *prompt target* — and the same ADR-028 says a modifier is never a prompt target, gets no Phase A or B, and has its raw events discarded before any lesson logic runs. The three rules are individually right and jointly unsatisfiable: the modifier's row is never written, so `á` never unlocks. B5's resolution closed the *drilling* question and left this one behind it. Candidate resolutions: give the modifier's introduction step an explicit "mark Active" write (needs a store method that creates a row without counting an attempt, which ADR-011 does not have); or redefine composite availability over *introduced* keys rather than Active ones (needs the introducer to persist what it introduced); or treat a composite's prerequisites as met when the base letter is Active and the modifier has been introduced in this profile's history. Pick one before the first dead-key language enters validation.

*Sharpened 2026-08-23 by alpha session 8, which built the introducer and did **not** take candidate 2.* Its record of what it has introduced is deliberately session-local ([ADR-023 § What an introduction step is, and what the introducer remembers](adr/0023-key-introduction-protocol.md#what-an-introduction-step-is-and-what-the-introducer-remembers)) — a key introduced but never answered is re-introduced next session, because the script is that key's only teaching moment. That is right for letters, which leave the record for good the moment they become Active, and it exposes a **second consequence of B8 that is worse than the filed one**: a modifier never becomes Active, so nothing suppresses it after the session it was announced in. The shipped Icelandic sequence re-announces the accent key at the head of the right-hand pool in *every* session — displacing a real right-hand key each time — and repeats it forever once the rest of the sequence is exhausted. So B8 does not only block composites from unlocking; it corrupts the introduction sequence from the modifier's step onward. Session 8 left this unfixed on purpose: every available repair is one of the three candidates above, and a fourth local rule would be the second definition of Active this entry warns against. `tests/test_introducer.py::TestModifierIntroduction::test_a_modifier_repeats_every_session_until_roadmap_b8_is_resolved` pinned the behaviour and was the marker to delete when the hole closed. *(Deleted 2026-08-23, alpha session 8c.)*

**B9. The introduction order is decided by an Accepted ADR that the project's own research had already superseded — and nothing linked the two.** *(Raised 2026-08-23, alpha session 8 follow-up.)*
*Phase: structurally resolved in Alpha; the ordering choice itself belongs to the Beta pilot. ADRs: [023](adr/0023-key-introduction-protocol.md), [010](adr/0010-lesson-structure-and-progression.md), [027](adr/0027-key-and-accuracy-state-model.md).*
[ADR-023](adr/0023-key-introduction-protocol.md) was accepted 2026-05-23 with a home-row-first Phase 1. [anchor-and-introduction-order.md](research/anchor-and-introduction-order.md) (2026-06-07) and its order-comparison spike (2026-06-14) both concluded that ordering is the weaker one — 9–10 keys before the first off-home reach against 2 for an F/J-seeded order. [alpha-plan.md](alpha-plan.md) was then written on 2026-06-21, *after* both, and scheduled the introducer as an implementation chunk with `Spec = ADR-023` and no reference to either document. Session 8 duly implemented the superseded model. **Root cause:** the plan was derived by walking the ADRs, and an audit of every research note found this one is the *only* one with zero inbound references from any ADR, the plan, the roadmap or architecture.md — every other note is wired into at least one decision document. The plan's safeguards could not have caught it: the tripwire fires on *under*specification, and this ADR was specified and stale; the carry-forward table is the right instrument but was populated from ADR open-questions lists; the findings sweep is retrospective.
**Structurally resolved 2026-08-23, and completed the same day by [ADR-032](adr/0032-grapheme-led-introduction-and-selectable-ordering.md)**, which lifts the ordering decision out of ADR-023 entirely and sets the three candidates out as siblings — amending the body again would have preserved the very framing that failed here. The order is a swappable strategy below a common Stage 0 ([ADR-023](adr/0023-key-introduction-protocol.md) § The introduction order is a swappable strategy), the ladder was de-positionalised so no milestone depends on "the home row" ([ADR-027](adr/0027-key-and-accuracy-state-model.md) § Milestone Ladder), and the note is linked from ADR-023 open question 6. **The ordering choice itself stays open and belongs to the pilot.**
**Process rule adopted:** a research note whose status is *pre-decision* and which contradicts an Accepted ADR must be linked from that ADR's open questions. The audit found exactly one violation, now closed.
...
- **Phase A vs Phase B counting** ([ADR-024](adr/0024-drill-content-and-lesson-granularity.md)): Phase A is "10 correct **in succession**" (reset on error) but Phase B is "20 correct with **no more than one rejection**" (cumulative-with-tolerance). Adjacent phases use different counting models — confirm intentional. *(Alpha)* **Session 9 implemented both literally**, so the question is now concrete rather than abstract: Phase A's streak returns to zero on any wrong first press, and Phase B counts a run of 20 correct that a *second* rejection restarts from zero (the first is the budget). Phase C is a third model again — a 30-attempt window re-taken whole if it lands under 85%. All three are per-member and pinned by `tests/test_drills.py::TestPhaseA`/`TestPhaseB`/`TestPhaseCAndD`; changing any of them is a threshold edit plus a test edit, nothing structural.
- **Does the curriculum wait for the anchor gate?** ([ADR-023](adr/0023-key-introduction-protocol.md) § Stage 0: "Before any curriculum runs, the child learns to find home by touch"; [ADR-027](adr/0027-key-and-accuracy-state-model.md) § The Anchor Gate makes completion a *measurement* — all six keys at 25 attempts, 95% and 2 distinct days.) Nothing in the code holds the sequence back: `KeyIntroducer.introduce_next()` hands out Stage 0's three steps and then the curriculum's, and session 9's drill generator ends a step when its ramp-up ends, which is a drill-content judgement and not the gate. So either the caller gates on the anchor rung — and then no child can leave the six anchor keys on their first day, since the gate needs two calendar days — or the stage is a strong opening rather than a barrier and the rung simply fires late. Both are defensible and the ADRs do not choose. *(Raised 2026-08-23, alpha session 9. Gates #10, which detects the rung, and #11, which drives `introduce_next`.)*
- **Keystrokes typed between prompts are dropped** ([ADR-027](adr/0027-key-and-accuracy-state-model.md) § First-Attempt Counting, as implemented in alpha session 7): `AttemptCounter` counts only against an open prompt, so a character arriving after one prompt is resolved and before the next is issued writes nothing and produces no feedback. Harmless in the single-threaded loop, where the engine issues the next prompt in the same tick — but it means a child typing ahead of the audio loses those keystrokes silently. Confirm the loop closes that window, or decide that type-ahead is accepted and counted. *(Alpha, session 11)*
- **Icelandic's grapheme denominator counts 32, but the physical layout carries 36** ([ADR-023](adr/0023-key-introduction-protocol.md) § Milestone denominators: "the denominator is the 32 output graphemes from `get_layout_positions()`"). 32 is the Icelandic *alphabet*; a real Icelandic keyboard also has C, Q, W and Z, and [ADR-007](adr/0007-language-data-word-frequency.md) makes the layout — not the alphabet — authoritative for the native set, which puts `build_is()` at 36 graphemes and 31 physical keys. Both rules are stated as decided and they disagree by four letters, so every rung of the milestone ladder shifts depending on which one is read. Decide whether a keyboard key that is outside the language's alphabet counts towards a milestone; the same question will recur for every language whose keyboard is a superset of its alphabet. *(Raised 2026-08-23, alpha session 8. Gates #10 milestone detection and #12's real layout reader; harmless for en/de, whose alphabets and keyboards coincide.)* **Session 8b built the ladder on the layout reading** — `grapheme_thresholds()` divides `len(layout.graphemes)`, which is 36 for `build_is()`, and `tests/test_milestones.py::TestLadderShape::test_icelandic_thresholds` pins 36 → 12/18/24/30/36. That test is the marker to revisit if the decision goes the other way; nothing else in the engine encodes the number.
...
- ~~**Sound-cue channel policy**~~ ✓ *Resolved 2026-08-22* — one reserved `pygame.mixer` channel per cue class, monophonic within a class, newest wins; mixer init pinned in `config.py`. Written up in [ADR-012 § Sound cue channel policy](adr/0012-audio-feedback-design.md#sound-cue-channel-policy). *(Alpha)*
