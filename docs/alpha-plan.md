...
| 4 | Audio out — `TTSEngine` (pyttsx3/SAPI) + `RecordingTTS`; `SoundCuePlayer` (`PygameMixerCues`) + `RecordingCues`; sound-cue channel/voice-stealing policy; concrete `LetterAudioSource` (SAPI-backed letters for Alpha en/de; curated-clip + per-profile-recording tiers deferred to Beta — Protocol + fake already landed 2026-07-05) | Sonnet | fake unit tests; real audio behind `audio` marker | [ADR-012](adr/0012-audio-feedback-design.md), [ADR-003](adr/0003-text-to-speech.md), [tts-letter-pronunciation](research/tts-letter-pronunciation.md) | ☐ | |
