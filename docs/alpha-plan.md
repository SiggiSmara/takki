...
|---|---|---|---|
| **Concurrency model** | #4, #6, #11 | Threading/event-loop model for pynput thread + pygame pump + blocking TTS + timers (design-review follow-up: define before #6) | **drafted (2026-07-05)** — [concurrency-model.md](concurrency-model.md): single-threaded core loop on the main thread, one inbound queue, TTS worker thread, deadline-checked timers, `TTSEngine.stop()` the only cross-thread call. Review/accept before #6. |
